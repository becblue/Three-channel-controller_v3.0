...
#### 3.1 系统控制模块（system_control）
- 目标：实现系统自检、主循环调度、状态机管理。
- 主要内容：
  - 上电自检（2秒LOGO、3秒进度条、输入/反馈/温度检测）
  - 主循环调度各功能模块
  - 系统状态机设计（自检、正常、异常、报警等）
  - 自检异常处理（N类异常）
- 注意事项：
  - 自检流程和判据严格按README
  - 状态切换要有详细注释
  - 所有状态变化通过DEBUG_Printf输出
- 测试方法：
  - 上电自检流程测试，串口和OLED显示同步验证
  - 故意制造自检异常，验证异常处理

#### 3.2 安全监控模块（safety_monitor）
- 目标：实现异常标志管理、报警输出、蜂鸣器控制、电源监控。
- 主要内容：
  - 异常标志（A~N、KLM等）检测与管理
  - 报警输出（ALARM引脚、蜂鸣器多种脉冲方式）
  - 电源监控（DC_CTRL异常检测）
  - 异常优先级和解除机制
- 注意事项：
  - 异常判据和报警方式严格按README
